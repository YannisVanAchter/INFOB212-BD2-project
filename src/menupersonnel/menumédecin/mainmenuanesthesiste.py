...
def main_anesthesiologist_menu(database: DataBase):
    ...
    id = get_valid_id(database, "What is your anesthesiologist ID?", "ANAESTHESIST") # Retrieve the anesthesiologist's ID
    ...
    while True:
        ...
        print("Type 3 if you want information about an organ you are going to transplant.")
        print("Type 4 if you want information about a specific client.")
        print("Type 5 or anything else if you want to stop requesting information from the database.")
...
def seepeople(database: DataBase, id):
    ...
    idT = get_int("What is the ID of the transplantation for which you want to see who you will be working with?")
...
def info_organe(database: DataBase):
    ...
    id_transplantation = int(input("Can you provide me with the ID of the transplantation for which you want to see the organs?"))

    organs_query = ("SELECT state, method_of_preservation, type FROM ORGAN WHERE id = '%s'")
...
def info_client(database: DataBase):
    ...
    """
    client = get_string("What is the ID of the patient for whom you want to get the information? ")
    database.connect()
    clients_query = ("SELECT Username, blood_type, blood_sign FROM CUSTOMER WHERE id = '%s'")
