...
def add_employee(db: DataBase):
    ...
    while existence != "yes" and existence != "no":
        existence = get_string(
            "Does this new employee has already an account PERSON? Enter no or yes "
        )
...
def create_employee(id, db: DataBase):
    ...
    description_person = get_string(
        "Please enter the description of the job of the person in the company: "
    )
    activity = True

    db.execute_with_params(
        "INSERT INTO STAFF (id, salary, job_description, active) VALUES (%s,%s,%s, %s)",
        (id, salary_person, description_person, activity),
    )
...
def delete_employee(db: DataBase):
    ...
    while confirmation != "yes" and confirmation != "no":
        confirmation = (
            get_string("Type yes if you want to delete this person or no otherwise: ")
            .strip()
            .lower()
        )
    ...
    if confirmation.startswith("y"):
        ...
        if id in medecins:
            if id in doctors:
                db.execute(
                    f"UPDATE DOCTOR SET inami_number = '-1' WHERE id = {id_employee}"
                )

            if id in anaesthetists:
                db.execute(
                    f"UPDATE ANAESTHESIST SET inami_number = '-1' WHERE id = {id_employee}"
                )
